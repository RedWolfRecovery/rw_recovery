...
int main(int argc, char** argv) {
  ...
  if (!status) {
    if (state.errmsg.empty()) {
      gui_msg("aborted_script=script aborted (no error message)");
    } else {
      gui_msg(Msg(msg::kError, "aborted_script_err=script aborted: {1}")(state.errmsg));
    ...
    gui_msg(Msg("done_script=script succeeded: result was [{1}]")(result));
  }
